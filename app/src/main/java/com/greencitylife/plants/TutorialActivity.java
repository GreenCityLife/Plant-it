...
public class TutorialActivity extends Activity {
	...
	private void initializeLogic() {
		_ARGHOZALITapTarget(imageview1, "This is the plant!", "Click this to increase points.", "#4CAF50");
		t = new TimerTask() {
			@Override
			public void run() {
				runOnUiThread(new Runnable() {
					@Override
					public void run() {
						sp.edit().putString("taptargets", "ok").commit();
						_ARGHOZALITapTarget(textview1, "Level", "This will display what level you play", "#4CAF50");
					}
				});
			}
		};
		...
		t = new TimerTask() {
			...
			public void run() {
				runOnUiThread(new Runnable() {
					@Override
					public void run() {
						_ARGHOZALITapTarget(textview2, "1 Minute Timer", "Finish your goal within the timer finishes to win!", "#4CAF50");
					}
				});
			}
		};
		...
		t = new TimerTask() {
			...
			public void run() {
				runOnUiThread(new Runnable() {
					@Override
					public void run() {
						_ARGHOZALITapTarget(textview5, "Heart", "You will have 3 hearts ❤ to win", "#4CAF50");
					}
				});
			}
		};
		...
		t = new TimerTask() {
			...
			public void run() {
				runOnUiThread(new Runnable() {
					@Override
					public void run() {
						_ARGHOZALITapTarget(textview3, "Goal", "Know, your goal here by your difficulty mode.", "#4CAF50");
					}
				});
			}
		};
		...
		t = new TimerTask() {
			...
			public void run() {
				runOnUiThread(new Runnable() {
					@Override
					public void run() {
						_ARGHOZALITapTarget(textview4, "Points", "Know, how much points you have taken", "#4CAF50");
					}
				});
			}
		};
		_timer.schedule(t, (int)(25000));
		t = new TimerTask() {
			@Override
			public void run() {
				runOnUiThread(new Runnable() {
					...
					public void run() {
						...
						startActivity(i);
						sp.edit().remove("taptargets").commit();
						finish();
					}
				});
			}
		};
		_timer.schedule(t, (int)(27000));
	}
	...
	public void onBackPressed() {
		if (sp.getString("taptargets", "").equals("ok")) {
			t.cancel();
			i.setClass(getApplicationContext(), LevelActivity.class);
			startActivity(i);
			sp.edit().remove("taptargets").commit();
	...
	public void _ARGHOZALITapTarget (final View _view, final String _title, final String _msg, final String _bgcolor) {
		TapTargetView.showFor(TutorialActivity.this,
